 ...
 * Values from mini() are strings; the render loop handles Number() coercion.
 */
...
describe("visual controls via createMixParam", () => {
  describe("as methods on Pattern", () => {
    it(".speed() merges speed into object-valued events", () => {
      const pat = src("a.mp4 b.mp4").speed("0.5 1");
    ...
    it(".scaleX() merges scaleX", () => {
      expect(query(src("x").scaleX("2"), 0).scaleX).toBe(2);
    });

    it(".scaleY() merges scaleY", () => {
      expect(query(src("x").scaleY("3"), 0).scaleY).toBe(3);
    });

    it("chaining multiple controls merges all keys", () => {
      const pat = src("clip.mp4").speed("2").alpha("0.5");
      const v = query(pat, 0);
    ...
    it("later control overrides earlier with same name", () => {
      const pat = src("x").speed("1").speed("2");
