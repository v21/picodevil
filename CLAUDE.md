...
- Smear: `.smear(angle, pixels, jitter=0.1)` = 5-tap directional Gaussian (angle in turns). Radius in screen pixels (px→UV like pixelate); **cost is fixed regardless of radius** (constant tap count). Implemented as `OP_SMEAR` ([src/effect-compiler.ts](src/effect-compiler.ts)) which *replaces* `OP_SAMPLE` in the sample-pivot slot when active (so `MAX_OPS` is unchanged, the common path stays a single tap). Jitter = a **per-tap positional x/y displacement** (Jenkins hash of `gl_FragCoord`+tap index, half-extent `jitter·|d1|`, applied to every sample incl. the centre) — dithers wide-offset aliasing into noise; taps wrap via `fract`. Static (no temporal term yet). Shader `smearSample()`/`pdJit()` in [src/webgl-renderer.ts](src/webgl-renderer.ts). Controls in [src/effects-controls.ts](src/effects-controls.ts). A box/2-D `.blur()` was dropped in favour of a planned `render()` primitive (bake chain-so-far to a hidden auto-FBO, à la `.modulate`), enabling a proper separable blur as `smear(0).render().smear(.25)`. `smearSample()` **filters in premultiplied alpha** (weights each tap's rgb by its alpha, then un-premultiplies) so transparent taps don't drag the average to black — no dark fringe/halo at alpha edges (barrel corners, contain/none letterbox, PNGs, `.render()` transparent regions); opaque input is bit-identical to a plain average. The per-tap screen-pixel width is also **footprint-consistent**: `cellW`/`cellH` derive from the render-target viewport (`currentViewW/H`), not the full canvas, so a `.render()` bake into a footprint sub-viewport smears the same screen width as a full-frame draw (regressions: [src/smear-screen-consistency.test.ts](src/smear-screen-consistency.test.ts), [src/smear-premult-alpha.test.ts](src/smear-premult-alpha.test.ts)). **Open**: temporal-jitter (fizzy vs textured) unexplored.
