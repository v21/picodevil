...
**Sources:** `mini(str)`, `color(str)`, `video(str)`, `image(str)`, `text(str)`, `qr(str)`, `screen(str)` / `s(str)`
...
`qr(str)` renders a QR code for `str` (usually a URL) as a source tile — **white modules on a transparent background** (the quiet zone is transparent too, so the black canvas shows through), error-correction level **Q** (~25%), default objectfit **`'contain'`** (fills the tile, square). Use single-quoted strings: a double-quoted URL is transpiled to `qr(mini("..."))` and the `/` parses as mininotation's slow operator (same gotcha as `.urlBase()`). Deliberately has **no QR-specific styling controls** — recolour/transform with the universal controls (`.brightness()`, `.tint()`, `.huerot()`, `.alpha()`, `.blend()`, `.width()`, `.grid()`, …); note the modules are white, so `.tint()` gives a starting colour that `.huerot()` alone can't shift. Modelled on `text()` but shares none of its path: implemented in `src/qr-pattern.ts`, rendering in `src/qr-render.ts` (uses the dependency-free `qrcode-generator` package), cached per-payload in `FrameRenderer.getQrCanvas` (LRU-capped at `MAX_QR_CANVASES`), with its own `{kind:'qr'}` `TileSource`.
...
- Visual: `.alpha()`, `.scale()`, `.scaleX()`, `.scaleY()`, `.objectfit()`, `.blend()`, `.grey()`, `.huerot(n)`, `.contrast(n)`, `.brightness(n)`, `.tint(hue, strength)`, `.barrel(k)`, `.smear(angle, pixels, jitter?)`, `.smearop(mode)`, `.blur(amount?)`, `.dilate(amount?)`, `.erode(amount?)` — huerot in [0,1] turns (0.5 = opposite hue); contrast centred at 0.5 (1=normal, 0=flat grey, -1=invert); brightness is additive offset (0=normal); tint attracts all pixels toward a hue (OKLab space, shared pass with huerot, unclamped strength for hyper-saturation); `.grey()` outside [0,1] adjusts saturation (negative = boost, >1 = chroma inversion); barrel k>0 = barrel distortion (CRT look, corners go transparent), k<0 = pincushion; applied in **cell-local** UV space (centred on the drawn cell, so per-tile on a crop/cropStack — see the renderer note above), before pixelation
- Smear (`.smear(angle, pixels=20, jitter=0.5)`): the general **9-tap multi-tap sampler**. **Sign of `pixels` = tap shape**: >0 linear (a directional 9-tap line), <0 circular (a rotated 9-tap ring), 0 jitter-only. `|pixels|` = radius; `angle` (turns) = line direction / ring rotation. The reducer over the taps is `.smearop` (default avg). One `OP_SMEAR` ([src/effect-compiler.ts](src/effect-compiler.ts)) covers both shapes + all reducers; it *replaces* `OP_SAMPLE` in the sample-pivot slot (so `MAX_OPS` unchanged). **Only one smear-family op per tile** — `.smear`/`.blur`/`.dilate`/`.erode` share the slot and override each other (last wins); combine via `.render()`. **Encoding is polar + shared aspect** (8-float op): `a.zw` = X-referenced tap vector (direction baked CPU-side in `smearFields`, so **no per-fragment trig** — ring taps step via `pdRot45`), `b.x` = jitter factor, `b.y` = flags (`shape·2 + sourceIsFbo`), `b.z` = aspect (`uvPerPxY/uvPerPxX`, signed → carries FBO/crop flips), `b.w` = reducer mode. The aspect scales the Y of every tap and of the jitter disc → the ring and the jitter are **screen-correct at any cell aspect**. **Jitter is a first-class blur**, not a dither: a uniform **disc** (`pdJitDisc`, was a square) of radius `jit·(|pixels|+1)·uvPerPx` — the `|pixels|+1` keeps it live at `pixels`=0 (jitter-only blur) and non-zero for a negative (ring) amount. `smearJitter` defaults to **0** in `buildTileParams`/`makeTile` (jitter only live when `.smear()` set it; its own default is 0.5), else every un-smeared tile would emit a stray jitter. Linear weights = a **σ=2 Gaussian** (`PD_GAUSS9`) at the same ±2·|s| reach as the old 5-tap — same look, denser. `smearSample()` **filters in premultiplied alpha** (transparent taps don't drag colour to black); FBO sources are un-premultiplied on read; taps wrap within the confine window from `OP_WRAP`. Regressions: [src/smear-screen-consistency.test.ts](src/smear-screen-consistency.test.ts), [src/smear-premult-alpha.test.ts](src/smear-premult-alpha.test.ts), [src/smear-fbo-source.test.ts](src/smear-fbo-source.test.ts). **Open**: temporal jitter unexplored.
- Smear reducer (`.smearop(mode)`): the reducer over the smear taps. Needs an active `.smear()`; a no-op alone. `mode` is a name (patternable) mapped to an int by `smearModeCode` ([src/effects-controls.ts](src/effects-controls.ts)) in `buildTileParams`, carried in `OP_SMEAR`'s `b.w`; `smearSample()` branches on it ([src/webgl-renderer.ts](src/webgl-renderer.ts)). Modes (no suffix = per-channel, `l` = luminance-keyed; synonyms in `SMEAR_MODE_CODES`): `avg`(0, default; `average`/`ave`/`mean`) / `avgl`(1), `max`/`dilate`(2) / `min`/`erode`(3), `maxl`(4)/`minl`(5), `median`/`med`(6) per-channel median (McGuire 9-tap `pdMed9` network) / `medianl`/`medl`(7), `range`/`edge`(8) per-channel max−min / `rangel`/`edgel`(9) coloured `abs(maxl−minl)`, `sharpen`/`sharp`(`sharpen1..9`/`sharp1..9` → 10..18, bare = `sharpen3`) unsharp.
- Morphology / blur aliases ([src/effects-controls.ts](src/effects-controls.ts), all thin wrappers over smear — no dedicated op): `.dilate(amount=3, jitter=0.5)` = `.smear(0, -|amount|, jitter).smearop('max')` (circular ring, per-channel max = bright grows); `.erode(...)` = same with `min`; `.blur(amount=8, jitter=0.5)` = `.smear(0, -amount, jitter)` (circular ring, avg = a soft bokeh blur). The negation goes through `reify(amount).fmap(v => -Math.abs(v))` so a patterned amount works. Per-channel max/min on straight rgb can tint fully-transparent edges (opaque video unaffected) — a follow-up.
- Texture modulation (`.modulate(source, amount=0.1)`, `.modspace('uv'|'tile'|'screen')`): Hydra-style UV displacement — the `source` pattern's pixels push the reading pattern's texture lookup around. `source` is **always** a pattern (wrap names/media in `s(...)`; a string arg throws a teaching error); it renders to a hidden auto-FBO named `__auto_mod_*` (registered via `nextAutoModName`/`registerAutoHidden` in [src/pattern-registry.ts](src/pattern-registry.ts), minted per call site, reset each eval). Shader op `OP_MODULATE` (kind 8) sits between BARREL and PIXELATE in [src/effect-compiler.ts](src/effect-compiler.ts) (`MAX_OPS=9`); semantics `uv += (mod.rg − 0.5) · amt · mod.a` (centred: mid-grey = no move; 0.5 bias hardcoded in the shader). **Shared semantics**: one written call = one modulator FBO, shared by all stacked instances (per-instance renders are a follow-up). Auto FBOs are stateless, so they're auto-sized to their consumers' on-screen footprint ([src/modulate-sizing.ts](src/modulate-sizing.ts) — pure footprint/ladder maths), rendered into a sub-viewport of a quantised-ladder texture, skipped entirely when no visible consumer reads them, and swept/recycled each frame in the renderer (`sweepAutoFBOs`, size-keyed free pool). Perf panel shows auto-FBO count + MB. Sizing maths in [src/modulate-sizing.ts](src/modulate-sizing.ts).
