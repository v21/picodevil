...
- **`WebGLRenderer`** (`src/webgl-renderer.ts`) — the only renderer. Uploads video frames as GPU textures (`gl.texImage2D`) — no CPU readback. UV rect and fit (cover/fill/tile/tilecenter) computed CPU-side; contain/none shrink the dest rect so the letterbox area is simply not drawn. Transforms (rotateZ, rotateX/Y scale, scaleX/Y) encoded as a 4×4 column-major matrix built on CPU. Blend modes use `blendFuncSeparate` so RGB and alpha channels accumulate correctly.
  - **UV effects run in cell-local space.** The fragment shader's working coord starts at `v_local` (the drawn quad's `[0,1]`); barrel/modulate/pixelate operate there, and the crop/fit map (`uvOffset`/`uvSize`, passed as flat varyings) is applied at `OP_WRAP` — the last step before the sample. So a UV effect is centred on and scaled to the visible cell, independent of where the crop window sits or how the `.render()` bake packs a footprint sub-viewport. This is why barrel on a `.cropStack()` tile (or after `.render()`) warps per-tile rather than around the source centre; to warp the whole frame before slicing, bake first (`.barrel(k).render().cropStack(...)`). `pixelate` step is in cell-local units (`pixels/cell`, no `uvSize` factor).
