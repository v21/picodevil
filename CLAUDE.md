...
```
uzuvid/
  index.html              — single-page app shell, loads main.ts via Vite
  main.ts                 — core runtime: pattern state, video pool, render loop, eval bridge
  editor.ts               — CodeMirror 6 editor setup, Ctrl+Enter eval binding, runs default code at startup
...
1. `main.ts` maintains a `pattern` (Strudel pattern object) and a `videoPattern` (VideoPattern | null).
...
Patterns use an explicit `.out()` method to push themselves as the active pattern (rather than returning from eval). Both `ColorPattern` and `VideoPattern` implement the `Outputable` interface. Internally, `.out()` calls a callback passed at construction time that sets the pattern on `main.ts` state.
...
- `setPlaybackRate()` in `playback-rate.ts` catches `NotSupportedError` to prevent the render loop from breaking
...
- Config constants live in `config.ts` — timing values are in milliseconds
